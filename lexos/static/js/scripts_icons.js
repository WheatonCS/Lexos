$(function () {
  ...
  $.each($('input[type=checkbox]'), function () {
    if (this.checked) { $(this).parent('label.icon-checkbox').addClass('checked') } else			{ $(this).parent('label.icon-checkbox').removeClass('checked') }
  })

	// Add initial classes for radios based on initial state (checked or unchecked)
  $.each($('input[type=radio]'), function () {
    if (this.checked) { $(this).parent('label.icon-radio').addClass('checked') } else			{ $(this).parent('label.icon-radio').removeClass('checked') }
  })
