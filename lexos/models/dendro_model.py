...
from lexos.models.matrix_model import MatrixModel, MatrixOption
...
class DendrogramModel(MatrixModel):
    def __init__(self, matrix_option: MatrixOption = None,
        ...
        :param dendro_option: the option for generating the dendrogram
        """
        super().__init__(matrix_option)

        self._dendro_option = dendro_option if dendro_option else \
    ...
    def _generate_dendrogram(self, dtm: pd.DataFrame) -> Figure:
    ...
    def generate_dendrogram_div(self, test_dtm: pd.DataFrame = None) -> str:
        """Generate the dendrogram div to send to the front end

        :return: a div
        """
        dtm = test_dtm if test_dtm else self.doc_term_matrix

        div = plot(self._generate_dendrogram(dtm), show_link=False,
